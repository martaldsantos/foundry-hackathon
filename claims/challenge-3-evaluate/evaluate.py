"""
Challenge 3: Evaluate — Claims Processing
...
def run_agent_on_dataset(dataset: list) -> list:
    """Run the claims triage agent on each test case and collect responses."""
    ...
    agent = client.agents.create_version(
        agent_name="eval-claims-agent",
        definition=PromptAgentDefinition(
            model=MODEL_DEPLOYMENT_NAME,
            instructions=(
                "You are an insurance claims triage agent. "
                "Analyze claim metrics and classify the claim status. "
                "Respond with: classification (normal/warning/critical), recommended action "
                "(approve/request_documents/investigate/deny), urgency level (low/medium/high), "
                "and reasoning."
            ),
        ),
    )
...
def run_evaluators(results: list):
    ...
    try:
        result = evaluate(
            ...
            evaluation_name="claims-triage-evaluation",
    ...
    if studio_url:
        ...
        print(
            ...
            "and look for: 'claims-triage-evaluation'"
        )

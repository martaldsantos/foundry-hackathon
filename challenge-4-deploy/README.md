...
Open [deploy.py](./deploy.py) and implement:

- **`ensure_agents_deployed()`** — list existing agents, create `anomaly-detection-agent` and `fault-diagnosis-agent` if not present
- **`run_anomaly_scan()`** — call the anomaly agent, handle the `check_thresholds` function call loop
- **`run_fault_diagnosis()`** — call the diagnosis agent for each affected machine
- **`run_factory_health_workflow()`** — orchestrate all steps and return the consolidated report

See [solutions/deploy.py](./solutions/deploy.py) if you get stuck.
